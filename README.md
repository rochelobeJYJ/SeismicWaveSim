...
웹 기반 지진파 시뮬레이터로, 지진파(P파 및 S파)의 전파 과정과 주시곡선 생성 과정을 실시간으로 시각화합니다. 이 프로젝트는 [FFLiK/SeismicWaveSimulator](https://github.com/FFLiK/SeismicWaveSimulator)의 원본 C++ 물리 엔진을 단일 HTML5/JavaScript 파일로 이식(Porting)한 버전으로, 현대적이고 직관적인 UI와 확장된 교육용 기능을 제공합니다.

## 🌟 주요 기능 (Key Features)

* **실시간 광선 추적 (Ray Tracing):** P파와 S파의 직접파, 반사파, 그리고 임계굴절파(선두파)의 전파 과정을 동시에 시뮬레이션합니다.
* **직관적인 지층 구조 편집:** 슬라이더 형태의 UI를 통해 다층 구조의 지구 내부 모델(두께, 밀도, Vp, Vs)을 실시간으로 설계하고 조작할 수 있습니다.
* **실시간 주시곡선 (Travel-Time Curves):** 이론적 곡선과 관측된 데이터 포인트를 동시에 매칭하여 보여주는 주시곡선 그래프를 실시간으로 렌더링합니다.
...
* **단일 파일 호환성:** 외부 서버나 복잡한 빌드 과정 없이, 오직 하나의 단일 HTML 파일로 모든 기능이 완벽하게 작동합니다.
